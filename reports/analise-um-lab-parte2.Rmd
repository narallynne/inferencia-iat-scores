...
iat %>% 
    group_by(sex) %>% 
    summarise(media = mean(d_art), desviop = sd(d_art), N = length(sex))
```
...
Em média, as mulheres que participaram do experimento tiveram uma associação implícita (medida pelo IAT) com a matemática *negativa* e *média* (média 0.5703113, desv. padrão 0.4229594, N = 65). Homens tiveram uma associação *negativa* com a matemática, todavia *maior* que a das mulheres (média 0.3997566, desv. padrão 0.5162869, N = 48). Houve então uma *pequena* diferença entre homens e mulheres (diferença das médias -0.1705546, 95% CI [-0.3430, 0.0093]). A partir desta amostra, estimamos que: (i) mulheres podem ter uma associação negativa forte; (ii) que pode não haver diferença entre sexos ou; (iii) que há uma possibilidade de homens terem associações negativas um pouco mais fortes ou moderadamente mais fortes para a matemática.
